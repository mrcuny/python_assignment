...
def exercise07(sd, m, s):
    ...
    # ------ Place code below here \/ \/ \/ ------
    random_numbers = np.random.normal(m, sd, size=s)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise13(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    start_date = '1/1/2010'
    date_range = pd.date_range(start_date, periods=n, freq='D')
...
def exercise14(words):
    ...
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(words)
    df = pd.DataFrame({'Length': s.map(lambda x: len(x))})
    # ------ Place code above here /\ /\ /\ ------
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df_e8 = pd.read_csv('https://tinyurl.com/y63q7okz')
    df = df_e8.iloc[::5, [2, 4]]
    # ------ Place code above here /\ /\ /\ ------
    return df
